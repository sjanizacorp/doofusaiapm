...
const { checkForUpdates, getInstalledChecks, getInstalledModules } = require('../engine/checkUpdater');
const fs = require('fs');
...
/**
 * @openapi
 * /api/v1/checks/registry:
 *   get:
 *     summary: Get complete check registry with installed status
 *     responses:
 *       200:
 *         description: Full check registry report
 */
app.get('/api/v1/checks/registry', asyncHandler(async (req, res) => {
...
/**
 * @openapi
 * /api/v1/checks/update-registry:
 *   post:
 *     summary: Fetch latest check metadata from upstream sources
 *     responses:
 *       200:
 *         description: Update report with new/changed/missing checks
 */
app.post('/api/v1/checks/update-registry', asyncHandler(async (req, res) => {
  const report = await checkForUpdates();
  res.json({ ...report, message: 'Registry refreshed from upstream sources' });
}));

/**
 * @openapi
 * /api/v1/checks/source/{checkId}:
 *   get:
 *     summary: Get the source code for a specific check module
 *     parameters:
 *       - in: path
 *         name: checkId
 *         required: true
 ...
 *     responses:
 *       200:
 *         description: Check module source code and metadata
 *       404:
 *         description: Check not found
 */
app.get('/api/v1/checks/source/:checkId', asyncHandler(async (req, res) => {
  const checkId = req.params.checkId;
  // Map policy ID to module filename
  const policies = getPolicies();
  const policy = policies.find(p => p.id === checkId);

  if (!policy) {
    return res.status(404).json({ error: `Policy ${checkId} not found` });
  }

  const modulePath = path.resolve(path.join(__dirname, '..', 'engine'), policy.check_module + '.js');
  if (!fs.existsSync(modulePath)) {
    return res.status(404).json({ error: `Check module not found at ${policy.check_module}.js` });
  }
  ...
  res.json({
    ...
    module_path: policy.check_module + '.js',
...
/**
 * @openapi
 * /api/v1/checks/updates:
 *   get:
 *     summary: Check for updates to frameworks and check definitions
 *     responses:
 *       200:
 *         description: Update report
 */
app.get('/api/v1/checks/updates', asyncHandler(async (req, res) => {
  const ENGINE_ROOT = path.join(__dirname, '..', 'engine');
  const report = await checkForUpdates(ENGINE_ROOT);
  res.json(report);
}));
 ...
 * /api/v1/checks:
 *   get:
 *     summary: List all installed checks with full details including source code
 *     parameters:
 *       - in: query
 *         name: include_code
 ...
 *     responses:
 *       200:
 *         description: Full check registry
 */
app.get('/api/v1/checks', asyncHandler(async (req, res) => {
  const ENGINE_ROOT = path.join(__dirname, '..', 'engine');
  const includeCode = req.query.include_code === 'true';
  const checks  = getInstalledChecks(ENGINE_ROOT);
  const modules = getInstalledModules(ENGINE_ROOT);

  const enriched = checks.map(c => {
    const mod = modules.find(m => m.id === c.id || c.check_module?.includes(m.file.replace('.js','')));
    return {
      ...c,
      module_file: mod?.file || null,
      module_size: mod?.size || null,
      last_modified: mod?.mtime || null,
      ...(includeCode && mod ? { source_code: mod.code } : {}),
    };
  });

  res.json({ checks: enriched, count: enriched.length, modules_count: modules.length });
}));
