...
// ─────────────────────────────────────────────
// SCAN CONFIG — 4-step wizard with family selector
// ─────────────────────────────────────────────
function ScanConfigScreen({ scans, onScanComplete }) {
  ...
  const selectFamily = (family) => {
    ...
    const ids = family.id === "all"
      ? ALL_CHECKS.filter(c => c.target_types.includes(targetType)).map(c => c.id)
      : ALL_CHECKS.filter(c => family.checkIds.includes(c.id) && c.target_types.includes(targetType)).map(c => c.id);
    setSelPolicies(ids);
  };
  ...
  const runScan = async () => {
    ...
    const applicable = ALL_CHECKS.filter(c => selPolicies.includes(c.id) && c.target_types.includes(targetType));

    // Generate realistic findings for demo
    const newFindings = applicable.slice(0, Math.min(applicable.length, 3)).map((c, i) => ({
      id: `F-${Date.now()}-${i}`,
      policy_id: c.id,
      policy_name: c.name,
      ...
      evidence: `Scan of ${form.name || targetType} — automated detection`,
      remediation: "See check documentation for detailed remediation steps.",
      framework_refs: c.frameworks.map(f => { const [fw, fid] = f.split(":"); return { framework: fw, id: fid }; }),
      first_seen: new Date().toISOString(),
    }));

    const postureScore = Math.max(10, 100 - newFindings.reduce((s, f) => s + f.score * 1.5, 0));
    const ns = {
    ...
    setLastScan(ns);
    setScanning(false);
    setStep(1); setTargetType(""); setSelectedFamily(null);
    setForm({ name:"", host:"", system_prompt:"" });
  };

  const familiesForTarget = CHECK_FAMILIES.filter(f =>
    f.id === "all" ||
    ALL_CHECKS.some(c => f.checkIds.includes(c.id) && c.target_types.includes(targetType))
  );
  ...
  return (
    <div style={{ maxWidth:860 }}>
      ...
      <Card style={{ marginBottom:24 }}>
        <SecTitle>Recent scans</SecTitle>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          <thead>
            <tr style={{ borderBottom:`1px solid ${C.border}` }}>
            ...
            {scans.slice(0, 7).map(s => (
              <tr key={s.id} style={{ borderBottom:`1px solid ${C.border}` }}>
                ...
                <td style={{ padding:"8px 10px", fontFamily:"monospace", fontSize:12,
                             color: s.posture_score ? (s.posture_score >= 70 ? C.healthy : s.posture_score >= 40 ? C.high : C.critical) : C.muted }}>
                  {s.posture_score ?? "—"}
                </td>
                <td style={{ padding:"8px 10px" }}><StatusBadge s={s.status} /></td>
      ...
      {lastScan && (
        <div style={{ background:"#0d2314", border:"1px solid #1a5a28", borderRadius:6,
                      padding:"12px 16px", marginBottom:20, fontFamily:"monospace", fontSize:12, color:C.healthy }}>
          ✓ Scan {lastScan.id} completed — posture score: {lastScan.posture_score} · {lastScan.findings} findings added to dashboard
        </div>
      )}
      ...
      <Card>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24 }}>
          {["Select target","Configure","Select check family","Review & run"].map((label, idx) => {
            const n = idx + 1;
            return (
              <div key={n} style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                {n < 4 && <span style={{ color:C.dim, fontSize:14 }}>›</span>}
              </div>
            );
          })}
        </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              {TARGET_TYPES.map(t => (
                <div key={t.id}
                     ...
                     style={{ background: targetType === t.id ? C.elevated : "transparent",
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {[{ key:"name", label:"Target name", ph:"e.g. prod-openai-gateway" }, { key:"host", label:"Host / endpoint URL", ph:"e.g. https://api.openai.com" }].map(({ key, label, ph }) => (
                <div key={key}>
              ...
              {targetType === "llm_api" && (
                <div>
                  ...
                  <textarea value={form.system_prompt} onChange={e => setForm(p => ({ ...p, system_prompt: e.target.value }))}
                            rows={3} placeholder="Paste system prompt here…"
              ...
              <div style={{ background:"#1a1000", border:"1px solid #3a2a00", borderRadius:4, padding:"10px 12px", fontFamily:"monospace", fontSize:11, color:"#d97706" }}>
                ⚠ API keys must be set as environment variables — never entered here. Use OPENAI_API_KEY, ANTHROPIC_API_KEY, etc.
              </div>
            </div>
        ...
        {/* Step 3 — check family selector */}
        {step === 3 && (
          <div>
            <div style={{ fontSize:13, color:C.muted, marginBottom:6, fontFamily:"monospace" }}>Which family of checks do you want to run?</div>
            <div style={{ fontSize:11, color:C.dim, fontFamily:"monospace", marginBottom:16 }}>Select a check family — individual checks can be fine-tuned in step 4.</div>
            <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:20 }}>
              {familiesForTarget.map(family => {
                const applicableCount = ALL_CHECKS.filter(c =>
                  (family.id === "all" || family.checkIds.includes(c.id)) && c.target_types.includes(targetType)
                ).length;
                ...
                return (
                  <div key={family.id}
                    ...
                    <div style={{ width:12, height:12, borderRadius:"50%", background: isSelected ? family.color : "transparent",
                                  border:`2px solid ${isSelected ? family.color : C.muted}`, flexShrink:0, marginTop:3 }} />
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, color: isSelected ? C.text : C.muted, fontWeight: isSelected ? 600 : 400, marginBottom:4 }}>
                        {family.label}
                        <span style={{ marginLeft:10, fontSize:10, fontFamily:"monospace", color: family.color,
                                       background: `${family.color}20`, border:`1px solid ${family.color}40`,
                                       padding:"1px 6px", borderRadius:3 }}>
                          {applicableCount} check{applicableCount !== 1 ? "s" : ""}
                        </span>
                      </div>
            ...
            <div style={{ display:"flex", gap:10 }}>
              ...
              <Btn variant="primary" onClick={() => setStep(4)} disabled={!selectedFamily}>Continue →</Btn>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div>
            <div style={{ fontSize:13, color:C.muted, marginBottom:16, fontFamily:"monospace" }}>
              Review checks ({selPolicies.length} selected from {selectedFamily?.label})
            </div>
            <div style={{ maxHeight:300, overflowY:"auto", display:"flex", flexDirection:"column", gap:6, marginBottom:16 }}>
              {ALL_CHECKS
                .filter(c => selPolicies.includes(c.id) && c.target_types.includes(targetType))
                .sort((a, b) => SEV_W[a.severity] - SEV_W[b.severity])
                .map(c => (
                  <div key={c.id}
                       onClick={() => setSelPolicies(prev => prev.includes(c.id) ? prev.filter(x => x !== c.id) : [...prev, c.id])}
                       style={{ display:"flex", alignItems:"center", gap:12, padding:"8px 12px",
                                background: selPolicies.includes(c.id) ? C.elevated : "transparent",
                                border:`1px solid ${selPolicies.includes(c.id) ? C.borderHi : C.border}`,
                                borderRadius:5, cursor:"pointer" }}>
                    <div style={{ width:14, height:14, borderRadius:3,
                                  background: selPolicies.includes(c.id) ? C.accent : "transparent",
                                  border:`1px solid ${selPolicies.includes(c.id) ? C.accent : C.border}`,
                                  display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                      {selPolicies.includes(c.id) && <span style={{ color:"#fff", fontSize:9 }}>✓</span>}
                    </div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:12, color:C.text }}>{c.name}</div>
                      <div style={{ fontSize:10, color:C.muted, fontFamily:"monospace", marginTop:2 }}>{c.id}</div>
                    </div>
            ...
            <div style={{ display:"flex", gap:10 }}>
              ...
              <Btn variant="success" onClick={runScan} disabled={scanning || selPolicies.length === 0}>
                {scanning ? "⟳ Scanning…" : `▶ Run Scan (${selPolicies.length} checks)`}
              </Btn>
            </div>
          </div>
        )}
...
export default function App() {
  ...
  // Called when a scan completes — adds scan + new findings to state
    ...
    <div style={{ display:"flex", minHeight:"100vh", background:C.bg, color:C.text, fontFamily:"'DM Mono','IBM Plex Mono','Fira Code',monospace" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
        ...
        <div style={{ padding:"28px 32px", overflowY:"auto", flex:1 }}>
          ...
          {page === "scans"     && <ScanConfigScreen scans={scans} onScanComplete={handleScanComplete} />}
